...
/// Devuelve el numero mas cercano a n de un intervalo. Es la identidad si el numero esta en el intervalo, o el extremo mas cercano si no esta.
float clip(float n, float lower, float upper) {
  return std::max(lower, std::min(n, upper));
}

/// Estructura que guarda parametros para los calculos de la proyeccion del rectangulo en la esfera
struct SphQuad{
  vec3 o, x, y, z, p;
  float z0, z0sq;
  float x0, y0, y0sq;
...
/// Inicializa los parametros de SphQuad, necesarios para los calculos posteriores y que inicializan la proyeccion del rectangulo en la esfera.
...
/// Genera un punto aleatorio en la proyeccion del rectangulo de forma uniforme respecto al angulo solido
